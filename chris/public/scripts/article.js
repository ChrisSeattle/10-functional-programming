...
(function(module) {
  ...
  Article.numWordsAll = () => {
    return Article.all.map(articleTemp => articleTemp.body.split('').length).reduce((count, word) => word + count);
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(articleTemp => articleTemp.author).reduce((uniqueList, current) => uniqueList.includes(current) ? uniqueList : [...uniqueList, current], ''); 
    // if current is already in uniqueList, return what we already have, else return an array of all elements of uniqueList and current. 
  };
